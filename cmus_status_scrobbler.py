	...
	def get_status_updates(self):
		...
		for row in cur:
			...
			if isinstance(su.cur_time, datetime.datetime):
				...
				status_updates[-1] = su._replace(
				    cur_time=su.cur_time.timestamp())
...
class Scrobbler:
	...
	def auth(auth_url, api_url, api_key, shared_secret, xml=False):
		...
		token = send_req(api_url,
		                 api_key,
		                 method=ScrobblerMethod.GET_TOKEN,
		                 xml=xml)
		if xml:
		...
		session = send_req(api_url,
		                   ...
		                   xml=xml)
		if xml:
			session = dict(key=session.split("<key>")[1].split("</key>")[0],
	...
	def make_scrobble(i, su):
		return {
		    f'artist[{i}]': su.artist,
		    f'track[{i}]': su.title,
		    f'timestamp[{i}]': str(int(su.cur_time)),
		    f'album[{i}]': su.album,
		    f'trackNumber[{i}]': su.tracknumber,
		    f'mbid[{i}]': su.musicbrainz_trackid,
		    f'albumArtist[{i}]':
		    su.albumartist if su.artist!=su.albumartist else None,
		    f'duration[{i}]': su.duration,
		}
	...
	def scrobble(self, status_updates):
		...
		batch_scrobble_request = reduce(lambda a, b: {
		    ...
		    **b
		    ...
		    Scrobbler.make_scrobble(i, su)
		    for (i, su) in enumerate(playing_sus)
		], dict(sk=self.sk))
...
def get_prefix_end_exclusive_idx(status_updates):
	...
	for i, (cur, prv) in enumerate(zip(r_su, r_su[1:])):
		if (cur.status==CmusStatus.stopped or not equal_tracks(cur, prv)
		    or cur.status==prv.status or prv.status==CmusStatus.stopped):
			return len(r_su)-i
